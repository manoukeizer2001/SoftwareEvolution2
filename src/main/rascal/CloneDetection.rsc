...
list[CloneResult] detectClones(list[Declaration] asts) {
    ...
    if (size(serializedAsts) > 0) {
        println("\nSample sequences from first AST:");
        list[str] firstSequence = serializedAsts[0][1];
        for (int i <- [0..min(5, size(firstSequence))]) {
            println("  Sequence at position <i>: <take(5, firstSequence[i..])>");
        }
    }
